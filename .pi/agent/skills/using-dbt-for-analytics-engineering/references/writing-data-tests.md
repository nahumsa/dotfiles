...
| Situation | Test | Why |
|-----------|------|-----|
| Enum column with known set of values found via discovery, documentation, or user-provided samples | `accepted_values` | Catches new invalid values |
| Non-PK column used in logic, discovery or user clarification confirmed 0% nulls | `not_null` | Catches regressions |
