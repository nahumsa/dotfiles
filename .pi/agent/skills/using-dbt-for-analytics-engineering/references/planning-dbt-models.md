...
Review the SQL, YAML, and unit test definitions to ensure that the model matches the requirements. If correctness depends on unknown data values or edge cases, ask the user for clarification or representative examples instead of executing project tooling.
